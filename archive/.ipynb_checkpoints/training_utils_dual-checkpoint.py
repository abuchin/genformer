...
def return_train_val_functions_hg(model,
    ...
    def dist_train_step(iterator_hg):
        ...
        def train_step_hg(inputs):
            target=tf.cast(inputs['exp'],dtype=tf.float32)
            sequence=tf.cast(inputs['inputs'],
                                 dtype=tf.bfloat16)
            atac =tf.cast(inputs['atac'],
                                 dtype=tf.bfloat16) ## need to pool to 128 bp res.
            tss =tf.cast(inputs['tss_tokens'],
                                 dtype=tf.bfloat16)
            exons =tf.cast(inputs['exon_tokens'],
                                 dtype=tf.bfloat16)
            introns =tf.cast(inputs['intron_tokens'],
                                 dtype=tf.bfloat16)
            tf_inputs = tf.cast(inputs['TF_exp'],
                               dtype=tf.bfloat16)  
            
            input_tuple = seq_inputs,tss,exons,introns,tf_inputs

            with tf.GradientTape() as tape:
                atac_out,rna_out = model(input_tuple,
                                         training=True)
                
                mse_loss = tf.reduce_sum(regular_mse(rna_out, target),
                                         axis=0)
                
                poisson_loss = tf.reduce_sum(poisson(atac_out,
                                                     atac),
                                             axis=0)

                loss = (mse_loss + poisson_loss) * (1. / global_batch_size)

                
            ### we will mod this to freeze the enformer layers
