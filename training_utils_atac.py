...
def return_train_val_functions(model,
    ...
    metric_dict["hg_val"] = tf.keras.metrics.Mean("hg_val_loss",
                                                  dtype=tf.float32)
    metric_dict["hg_corr_stats"] = metrics.correlation_stats_aformer(name='hg_corr_stats')
    ...
    def dist_train_step(iterator):
        ...
        def train_step_hg(inputs):
            ...
            seq_inputs=tf.cast(inputs['inputs'],
                                 dtype=tf.bfloat16)
            target =tf.cast(inputs['atac'],
                                 dtype=tf.bfloat16)
            tf_input = tf.cast(inputs['TF_acc'],
                               dtype=tf.bfloat16)  

            input_tuple = seq_inputs,tf_input
            ...
            with tf.GradientTape() as tape:
                with tf.GradientTape() as input_grad_tape:
                    ...
                    loss = tf.reduce_sum(poisson(output, target),
        ...
        for _ in tf.range(train_steps): ## for loop within @tf.fuction for improved TPU performance
            lr_rep,it_rep=strategy.run(train_step_hg, args=(next(iterator),))
    ...
    def dist_val_step(iterator):
        ...
        def val_step(inputs):
            ...
            metric_dict["hg_corr_stats"].update_state(target,
                                                      output)

            return target, output#, cell_type
        

        for _ in tf.range(val_steps): ## for loop within @tf.fuction for improved TPU performance
            target_rep, output_rep = strategy.run(val_step,
                                                                args=(next(iterator),))
...
def deserialize(serialized_example,input_length, 
    ...
    atac = tf.reshape(atac, [-1,128])
    #print(atac)
    atac = tf.reduce_sum(atac, axis=1)
    #print(atac)
    ...
    return {
        'inputs': tf.ensure_shape(sequence,[input_length,4]),
        'atac': tf.ensure_shape(atac, [input_length // 128,1]),
        'TF_acc': TF_acc
    }
