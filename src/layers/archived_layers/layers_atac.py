...
class headmodule_block(kl.Layer):
    def __init__(self,
        ...
        self.tf_bottleneck = kl.Dense(self.bottleneck_units_tf,
                                    ...
                                    kernel_regularizer=regularizers.L2(self.kernel_regularizer),
                                    use_bias=False)
        self.dropout_tf = kl.Dropout(rate=self.dropout_rate / 4,**kwargs)
        ...
        self.one_by_one = conv1Dblock(num_channels=1,
                                        conv_filter_size=1, 
                                        stride=1, 
                                        kernel_regularizer=self.kernel_regularizer,
                                        momentum=self.momentum,
    ...
    def call(self, inputs, training=None):
        ...
        tf_out = tf.expand_dims(tf_out,axis=1)
        tf_out = tf.keras.backend.repeat_elements(tf_out, 
                                                  x.shape[1], 
                                                  axis=1)

        final_out = tf.concat([x,tf_out],2)
        
        final_out = self.one_by_one(final_out,training=training)
