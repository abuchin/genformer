...
# # todo: remove explicit bfloat16 casting


def create_projection_matrix(m, d, seed, scaling=0, struct_mode=False):
    ...
    for _ in range(nb_full_blocks):
        if struct_mode:
            ...
            unstructured_block = tf.random.normal((d, d), seed=current_seed,dtype=tf.float16)
    ...
    final_matrix = tf.cast(tf.experimental.numpy.vstack(block_list),dtype=tf.bfloat16)
    current_seed += 1

    if scaling == 0:
        multiplier = tf.norm(tf.random.normal((m, d),dtype=tf.bfloat16), axis=1)
    elif scaling == 1:
        multiplier = tf.math.sqrt(tf.cast(d,dtype=tf.bfloat16)) * tf.ones((m))
    else:
...
def create_products_of_givens_rotations(dim, seed):
    ...
    for _ in range(nb_givens_rotations):
        random_angle = math.pi * tnp.random.uniform()
        random_indices = tnp.random.choice(dim, 2)
    ...
    return tf.cast(tf.constant(q),dtype=tf.bfloat16)
...
def relu_kernel_transformation(data,
    ...
    #else:
    ratio = 1.0 / tf.math.sqrt(tf.cast(projection_matrix.shape[0],dtype=tf.bfloat16))
...
def softmax_kernel_transformation(data,
    ...
    """
    data_normalizer = 1.0 / (
        tf.math.sqrt(tf.math.sqrt(tf.dtypes.cast(data.shape[-1], tf.bfloat16))))
    data = data_normalizer * data
    ratio = 1.0 / tf.math.sqrt(
        tf.dtypes.cast(projection_matrix.shape[0], tf.bfloat16))
...
def noncausal_numerator(qs, ks, vs):
    ...
    return tf.einsum("lbhm,bhmd->lbhd", qs, kvs),qs,ks
...
def favor_attention(query,
    ...
    key_prime = kernel_transformation(key, False, projection_matrix)  # [B,L,H,M] # return and compute Q' K'^T to get B X L X L x H tensor, normalize across rows to sum 1
    ...
    av_attention,q_pr,k_pr = noncausal_numerator(query_prime, key_prime, value)
...
class Attention(tf.keras.layers.Layer):
    ...
    def build(self, input_shape):
        ...
        self.projection_matrix = create_projection_matrix(
            self.nb_random_features, size_per_head, seed=seed)
    ...
    def call(self,
        ...
        attention_output,k_prime,q_prime = favor_attention(query, key, value,
                                                           kernel_transform, 
                                                           self.causal,
...
class SelfAttention(Attention):
    ...
    def call(self,
        ...
        return super(SelfAttention, self).call(query_input,
                                               query_input,
                                               training,
                                               cache, 
                                               decode_loop_step)
