...
def return_train_val_functions(model,
    ...
    def dist_train_step(inputs):
        ...
        with tf.GradientTape() as tape:
            ...
            target_atac = tf.gather(target_atac, mask_indices,axis=1)/100.0
            output_atac = tf.gather(output_atac, mask_indices,axis=1)/100.0
...
def deserialize_tr(serialized_example, g, use_tf_activity, input_length = 196608,
    ...
    rna_lookup = {0:1.5, 1:1.5,2:1.5, 3: 1.5,
    ...
    weighting_factor=tf.expand_dims(weighting_factor,axis=0)
...
def parse_args(parser):
    ...
    parser.add_argument('--atac_scale',
                        dest='atac_scale',
                        type=str,
                        default="0.0001",
                        help= 'atac_scale')
