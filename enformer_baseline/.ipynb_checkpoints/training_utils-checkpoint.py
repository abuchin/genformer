...
def deserialize_tr(serialized_example,input_length,max_shift, out_length,num_targets, g):
    ...
    diff = tf.math.sqrt(tf.nn.relu(target_cage - 2000.0 * tf.ones(target_cage.shape,dtype=tf.float16)))
    target_cage = tf.clip_by_value(target_cage, clip_value_min=0.0, clip_value_max=2000.0) + diff
    target = target_cage
